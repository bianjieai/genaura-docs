...
![内容发布](./assets/content-publish/01-publish.png)
...
> 发布功能使用的是 GenAura 内嵌浏览器的登录态，与系统浏览器不共享。若平台显示「未登录」，可在发布对话框点击「前往登录」打开内嵌登录窗口，登录成功后窗口会自动关闭并提示「<平台> 登录成功」。
...
点击任意平台卡片的「**发布记录 (N)**」按钮，打开该平台对此文章的历史发布记录弹窗（标题「<平台> 发布记录」）：
...
发布使用的是 GenAura 内嵌浏览器的登录态，与系统浏览器不共享。请在发布对话框点击该平台的「前往登录」打开内嵌登录窗口，登录成功后窗口会自动关闭并提示「<平台> 登录成功」。
