...
简曜内置了一个浏览器，让你无需切换到外部浏览器即可在应用内访问任意网站。它常用于：
...
**Q4：内嵌浏览器和系统浏览器（如 Chrome）的登录态是共享的吗？** A：不共享。内嵌浏览器是 简曜简曜内部的独立浏览器环境，登录态只在内嵌浏览器中保留。若你想在系统浏览器中访问同一账号，需要重新登录。
...
**Q7：关闭浏览器后再次打开同一网页，登录态还在吗？** A：在。简曜会持久化保存登录态，关闭浏览器只是结束当前会话，不会清除 Cookie。下次打开同一平台仍处于登录状态。
