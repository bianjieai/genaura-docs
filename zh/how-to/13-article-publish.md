...
简曜提供从知识库 Markdown 文章到多个内容平台的一键发布能力。在文件编辑器中打开一篇文章，点击「发布」按钮，系统会自动检测你在各平台的登录状态，并将文章投递到百家号、CSDN、搜狐号、掘金、知乎等多个内容平台。发布完成后，结果会记入发布记录，并根据平台是否需要审核自动跟踪发布状态。
...
- 已登录简曜，并已在某个品牌工作区下打开知识库中的 Markdown 文件（`.md`）。发布入口仅在文件编辑器视图中可见，知识库列表或对话预览不提供发布按钮。
