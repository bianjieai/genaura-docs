...
简曜提供从知识库 Markdown 文章到多个自媒体内容平台的一键自动化代发能力。在文件编辑器中打开一篇文章点击「发布」，系统会弹出「选择发布方式」对话框。选择**「自有账号发布」**后，系统会自动检测你在各平台的登录状态，并将文章快速同步投递到百家号、CSDN、搜狐号、掘金、知乎等多个平台。发布完成后，结果会自动记入发布记录，并根据平台是否需要审核自动跟踪发布状态。
...
- 从文件编辑器进入发布分流弹窗，选择自有账号发布模式
- 识别各平台的内嵌登录状态并进行补登
- 单平台或批量发布文章到百家号、CSDN、搜狐号、掘金、知乎等
...
> 提示：
> - 如果你需要采买全网高权重门户媒体或行业垂直自媒体大号进行权威背书与外推，无需自备账号，请查阅 [第三方媒体采买与履约发布](/zh/how-to/17-media-delivery)。
> - 如果你想了解发布双轨机制的底层逻辑，请阅读 [媒体发布与全渠道投放模型](/zh/concepts/06-media-delivery)。
> - 如果你想跑通「洞察 → 生成文章 → 编辑 → 发布」的完整端到端流程，请阅读 [从洞察到发布文章](/zh/tutorials/03-content-publish)。

## 前置条件

- 已登录简曜，并已在某个品牌工作区下打开知识库中的 Markdown 文件（`.md`）。发布入口仅在文件编辑器视图及知识库列表操作菜单中可见。
- 已在简曜内嵌浏览器中登录过目标发布平台。简曜发布使用的是内嵌浏览器的登录态，与系统浏览器不共享。若未登录，可在发布对话框中点击「前往登录」补登。
...
1. 在右侧面板切换到 **知识库** Tab，点击列表中的 `.md` 文件节点进入文件编辑器（或在列表项右侧菜单点击「发布」）。
2. 在编辑器顶部操作区，点击带纸飞机图标的 **发布** 按钮。
3. 系统首先弹出 **「选择发布方式」** 对话框，展示「自有账号发布」与「第三方媒体代发」两个卡片。
4. 点击 **「自有账号发布」** 卡片（或点击卡片右下角的「立即前往」按钮），系统将自动读取文章并打开「发布文章到内容平台」对话框。若文件读取失败，对话框不会打开。
...
- **标题**（可点击）：有标题时显示标题（下方小字显示截断链接）；无标题时直接显示截断链接。点击行为：
  ...
  - 其他状态 → 在简曜内嵌浏览器窗口打开（优先使用预览链接，回退到正式发布链接）。
- **状态标签**：见步骤 5.4。
...
- **基于已有素材迭代文章**：在知识库中选中已有 `.md` 文件，选中段落点击「添加至对话」，再输入 `/` 选择「生成 GEO 文章」，AI 会结合被引用片段生成新版本。详见 [命令与提及](/zh/how-to/05-commands-mentions)。
...
**Q2：平台显示「未登录」，但我在系统浏览器里已经登录过。** A：简曜发布使用的是内嵌浏览器的登录态，与系统浏览器不共享。请在发布对话框点击该平台的「前往登录」打开内嵌登录窗口，登录成功后窗口会自动关闭并提示「{平台} 登录成功」。

**Q3：发布卡在「发布中...」超过 5 分钟。** A：单次发布最长等待 5 分钟，超时会提示「投放操作超时（5分钟）」。常见原因是目标平台异常或网络不通，请重试；如持续失败请检查平台账号权限或更换网络环境。

**Q4：发布成功但状态显示「审核中」一直不变。** A：搜狐号、掘金这类需要审核的平台，文章需平台人工审核。简曜会周期性检查发布链接是否可达且标题匹配，匹配成功则自动转为「已发布」。为避免频繁请求，检查设有冷却期；累计检查达到上限仍未确认则显示「未确认」，此时可在发布记录的状态气泡中手动「标记为已发布」。
...
## 相关文档
 ...
 - 端到端教程：[从洞察到发布文章](/zh/tutorials/03-content-publish) — 洞察 → 生成 → 编辑 → 发布的完整链路。
 - 商业采买：[第三方媒体采买与履约发布](/zh/how-to/17-media-delivery) — 全网高权重信源采买与履约发布。
 - 核心概念：[媒体发布与全渠道投放模型](/zh/concepts/06-media-delivery) — 双轨发布机制与全渠道数据模型。
 - 操作指南：
   - [偏好洞察](/zh/how-to/16-preference-insight) — 投放文章追踪与 AI 引用频次监测。
   - [知识库](/zh/how-to/08-knowledge-base) — GEO 内容系统目录、文件导入与搜索。
   - [文件编辑器](/zh/how-to/09-file-editor) — 编辑器能力、可编辑/预览类型、导出格式、自动保存与草稿恢复。
   - [内嵌浏览器](/zh/how-to/10-browser) — 内嵌浏览器与登录态管理。
   - [命令与提及](/zh/how-to/05-commands-mentions) — `/` 命令生成 GEO 文章。
 - 故障排查：[故障排查](/zh/troubleshooting) — 「发布失败」「登录态过期」相关章节。
