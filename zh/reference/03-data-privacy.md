...
详见 [品牌管理](/zh/how-to/01-brand-management)。
...
切换简曜账号时，应用**不会自动清除**已登录的 AI 平台 Cookie。如果需要在某 AI 平台切换账号，请在内嵌浏览器中手动登出原账号后重新登录。详见 [内嵌浏览器](/zh/how-to/10-browser)。
...
如果确实需要重建，可以删除整个品牌目录后重新创建品牌，让 AI Agent 重新生成。详见 [知识库管理](/zh/how-to/08-knowledge-base)。
...
> `.csv` / `.xlsx` / `.xls` / 图片等类型只能在 [文件编辑器](/zh/how-to/09-file-editor) 中预览，不能导入知识库参与检索。
...
5 个 AI 平台的登录态统一保存在用户数据目录下的浏览器数据分区中，所有平台共用同一分区。切换简曜账号时**不会**自动清除已登录的 AI 平台 Cookie。如需在某 AI 平台切换账号，请在内嵌浏览器中手动登出原账号后重新登录。

### Q3：删除品牌时，对应的知识库物理文件会一起删除吗？

会。删除品牌操作会同步清理该品牌对应的整个知识库目录与数据库中该品牌的全部记录，包括知识库物理文件、对话、机会风险等。详见 [品牌管理](/zh/how-to/01-brand-management)。
...
- [设置](/zh/reference/01-settings) — 通用设置、关于、退出登录入口
- [更新与维护](/zh/reference/02-update-maintenance) — 自动更新机制与磁盘空间守护
- [登录与账号](/zh/getting-started/02-login-account) — 登录认证、账号体系、退出登录
- [知识库管理](/zh/how-to/08-knowledge-base) — 知识库导入、目录结构、检索、发送到对话
- [文件编辑器](/zh/how-to/09-file-editor) — 文件预览类型、导出格式、草稿恢复
- [内嵌浏览器](/zh/how-to/10-browser) — 浏览器接管、登录态管理
- [AI 平台工作流](/zh/how-to/11-ai-platform-workflows) — 5 个 AI 平台工作流运行机制
