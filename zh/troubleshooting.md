...
本篇按**你能感知到的症状**组织简曜使用过程中的常见故障，每个症状给出**可能原因**与**解决步骤**，便于在出现问题时快速对号入座并自助排查。
...
> 阅读建议：先按"症状"对号入座，再按"解决步骤"逐项操作；若仍无法解决，请参考末尾"相关文档"深入对应模块文档，或前往[常见问题](/zh/faq)查看是否有同款疑问。

## 前置条件

- 已按 [安装指南](/zh/getting-started/01-installation) 完成安装并首次启动。
- 已按 [登录与账号](/zh/getting-started/02-login-account) 登录账号并进入工作区。
- 出现问题前应用处于正常工作状态（未提示磁盘空间不足、未崩溃）。
- 出现工作流相关问题时，请先确认内嵌浏览器中目标 AI 平台已登录（见 [内嵌浏览器](/zh/how-to/10-browser) 的登录态管理）。
...
> 进一步阅读：[登录与账号 - 常见问题](/zh/getting-started/02-login-account#常见问题)。
...
2. 若是平台改版导致，需等待简曜更新工作流定义（由维护者处理）。
...
> 进一步阅读：[AI 平台工作流](/zh/how-to/11-ai-platform-workflows)、[工作流原理 - 接管机制](/zh/concepts/02-workflow)。
...
3. 仍不行时，参考 [内嵌浏览器 - 登录态管理](/zh/how-to/10-browser) 重新建立登录态。

> 进一步阅读：[内嵌浏览器 - 浏览器接管对话框](/zh/how-to/10-browser)、[工作流原理 - 接管机制](/zh/concepts/02-workflow#接管机制)。
...
> 注意：csv / xlsx / xls / 图片等格式只能在 [文件编辑器](/zh/how-to/09-file-editor) 中**预览**，不能导入知识库。
...
> 进一步阅读：[知识库 - 导入本地文件 / 导入 URL / 常见错误](/zh/how-to/08-knowledge-base)。
...
> 说明：简曜 发布使用的是内嵌浏览器的独立会话，与系统浏览器不共享，必须在发布对话框内重新登录。
...
**可能原因**：搜狐号、掘金这类需要人工审核的平台，文章需平台审核完成后才会转为「已发布」。简曜会周期性检查发布链接是否可达且标题匹配，匹配成功则自动转为「已发布」。
...
**说明**：自有账号发布的失败记录**不会写入发布记录**，只在发布完成时的提示框中显示「{平台}: {错误信息}」。常见原因为登录态过期或平台 API 异常。

### 症状 G：第三方媒体代发提示「文章标题长度不能超过 200 字」或「正文长度不能超过 200,000 字」

**可能原因**：触发了前置内容强校验规则。第三方媒体网络接口对稿件字符数有硬性限制。

**解决步骤**：
1. 检查文章一级标题或元数据，精简多余副标题，保证标题字符数 ≤ 200 字；
2. 检查正文字符数（去除多余的数据表或拆分为多篇文章），保证字符数 ≤ 200,000 字；
3. 保存后重新点击发布并选择「第三方媒体代发」。

### 症状 H：收银台支付宝扫码提示「二维码已失效，请刷新」或支付超时

**可能原因**：为保障价格时效与订单幂等性，每个收银台动态二维码有效期为 5 分钟。超过 5 分钟未支付订单自动取消。

**解决步骤**：
1. 直接在收银台弹窗中点击「刷新二维码」，系统将重新生成最新付款二维码；
2. 购物车中的选品与文章快照不会丢失；
3. 使用支付宝重新扫码完成结算。

### 症状 I：第三方媒体订单显示「发布失败 / 已拒绝」，款项退到哪里了？

**可能原因**：外部媒体编辑审核认为文章营销色彩过浓、存在敏感词、版面售罄或超过最长审发时效。

**解决步骤**：
1. 在右侧面板「AI 信源」下方的「投放文章追踪」中找到该条记录，点击状态标签查看媒体编辑返回的具体拒稿原因；
2. **款项自动原路退回**：系统会在置为拒稿的同时自动发起全额原路退款，资金将退回原支付支付宝账户（通常即刻或数分钟内到账），无需联系客服提工单；
3. 根据拒稿建议在知识库中优化文章后，可再次发起选品采买。

### 症状 J：选品工作台弹出「检测到重复投放」确认弹窗

**可能原因**：系统防重机制检测到当前文章在历史记录中已经为该媒体采买出稿过，或正在履约中。

**解决步骤**：
1. 若系无意误选，点击「取消添加」避免重复消耗采买预算；
2. 若确实需要对该媒体补发或增投矩阵号，可点击「坚持投放」继续进行发布。
...
3. 清理简曜知识库中不再使用的文件（特别是大体积 PDF/DOCX 原件）。
...
> 进一步阅读：[更新与维护参考 - 磁盘空间告警](/zh/reference/02-update-maintenance#磁盘空间告警)。
...
1. 打开任务管理器（Windows）或活动监视器（macOS），结束所有简曜进程后重试。
2. 重启电脑后再次尝试启动。
...
1. **首次启动右键打开**：在「应用程序」中右键点击简曜→「打开」→ 确认框中点击「打开」（仅需一次）。
...
> 进一步阅读：[安装指南 FAQ](/zh/getting-started/01-installation#常见问题)。
...
> 进一步阅读：[消息发送 FAQ](/zh/how-to/04-messaging#常见问题)。
...
- 入门：
  ...
  - [登录与账号](/zh/getting-started/02-login-account)（含登录常见问题）
  - [界面总览](/zh/getting-started/04-interface-overview)
- 概念：
  - [工作流原理](/zh/concepts/02-workflow)（异常检测、接管机制、工作流中止与恢复）
  - [AI Agent 协作模型](/zh/concepts/04-ai-agent)（子代理与人工审批）
- 操作指南：
  - [知识库](/zh/how-to/08-knowledge-base)（导入限制、URL 导入常见错误）
  - [文件编辑器](/zh/how-to/09-file-editor)（可编辑/预览类型）
  - [内嵌浏览器](/zh/how-to/10-browser)（接管对话框、登录态管理）
  - [AI 平台工作流](/zh/how-to/11-ai-platform-workflows)（运行、接管、结果）
  - [定时任务](/zh/how-to/12-cron-tasks)（周期性运行工作流）
  - [文章一键发布](/zh/how-to/13-article-publish)（发布状态检查机制、常见问题）
- 参考：
  - [设置](/zh/reference/01-settings)
  - [更新与维护](/zh/reference/02-update-maintenance)（自动更新、磁盘空间告警）
  - [数据隐私](/zh/reference/03-data-privacy)
- 常见问题：[faq.md](/zh/faq)
